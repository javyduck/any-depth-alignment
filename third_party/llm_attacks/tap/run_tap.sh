...
# TAP Attack Batch Runner
# Runs TAP attacks on multiple models using both advbench and jailbreakbench datasets with FIFO GPU scheduling
# Usage: ./run_tap.sh
export OPENAI_API_KEY="${OPENAI_API_KEY:?set OPENAI_API_KEY in your environment}"

# Configuration
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ATTACK_SCRIPT="$SCRIPT_DIR/attack_tap.py"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "This script runs TAP attacks on the following models:"
...
# TAP Parameters (based on original paper settings)
ATTACK_MODEL="gpt-4.1-2025-04-14"
EVALUATOR_MODEL="gpt-4o"
BRANCHING_FACTOR=2      # Paper setting: 4 (not default 1)
WIDTH=5               # Paper setting: 10
DEPTH=5               # Paper setting: 10
N_STREAMS=1            # Paper setting: 1 (implied)
KEEP_LAST_N=3          # Default: 3
MAX_TOKENS=3000        # Updated to 3000
SEED=42
...
echo "[INFO] Using evaluator model: $EVALUATOR_MODEL"
echo "[INFO] TAP Parameters:"
echo "[INFO]   - branching_factor=$BRANCHING_FACTOR (paper setting)"
echo "[INFO]   - width=$WIDTH, depth=$DEPTH"
echo "[INFO]   - n_streams=$N_STREAMS, keep_last_n=$KEEP_LAST_N"
echo "[INFO]   - max_tokens=$MAX_TOKENS, seed=$SEED"
...
if [[ $completed_count -eq ${#job_queue[@]} ]]; then
  ...
  echo "Results saved to: $RESULTS_DIR/TAP/"
else
